...
router.post('/get_requested_friends', verify, async (req, res) => {
  ...
  if ( index === undefined|| count === undefined)
    return callRes(res, responseCode.PARAMETER_IS_NOT_ENOUGH, ': index, count');
  if (!checkInput.checkIsInteger (index) || !checkInput.checkIsInteger (count))
    return callRes(res, responseCode.PARAMETER_TYPE_IS_INVALID, ': index, count');
  if (index < 0 || count < 0) return callRes(res, responseCode.PARAMETER_VALUE_IS_INVALID, ': index, count');
  ...
  try {
    ...
    if (data.request.length == 0) return callRes(res, responseCode.NO_DATA_OR_END_OF_LIST_DATA);
    thisUser = await User.findById(id);
...
router.post('/set_request_friend', verify, async (req, res) => {
  ...
  if (user_id === undefined)
    return callRes(res, responseCode.PARAMETER_IS_NOT_ENOUGH, 'user_id');
  if (typeof user_id != 'string')
    return callRes(res, responseCode.PARAMETER_TYPE_IS_INVALID, 'user_id');
  let id = req.user.id;
  let targetUser, thisUser;
  if (id == user_id) {
    return callRes(res, responseCode.PARAMETER_VALUE_IS_INVALID, 'user_id');
  } else {
    try {
      targetUser = await User.findById(user_id);
      if (!targetUser) return callRes(res, responseCode.NO_DATA_OR_END_OF_LIST_DATA, 'targetUser');
      thisUser = await User.findById(id);
      if(!thisUser) return callRes(res, responseCode.NO_DATA_OR_END_OF_LIST_DATA, 'thisUser');
      if(thisUser.friends.length >= MAX_FRIEND_NUMBER)
        return callRes(res, responseCode.NO_DATA_OR_END_OF_LIST_DATA, 'out of Max Friends');

      let indexExist = thisUser.friends.findIndex(element => element.friend._id.equals(targetUser._id));
      if (indexExist >= 0)
        return callRes(res, responseCode.ACTION_HAS_BEEN_DONE_PREVIOUSLY_BY_THIS_USER, 'you two are friend');
      ...
      if (findIndexBlockTarget >= 0) return callRes(res, responseCode.NOT_ACCESS, 'bị block rồi em ơi ko gửi kb được');
...
router.post("/set_block", verify, async(req, res) => {
    ...
    if (thisUser.isBlocked){
        return callRes(res, responseCode.USER_IS_NOT_VALIDATED, 'Tài khoản của bạn đã bị chặn');
    }
    if (!token || !user_id || !type){
        return callRes(res, responseCode.PARAMETER_IS_NOT_ENOUGH, 'token and user_id and type');
    }
    if (type != 1 && type != 0){
        return callRes(res, responseCode.PARAMETER_VALUE_IS_INVALID, 'type');
    }
    if (user_id == id){
        return callRes(res, responseCode.PARAMETER_VALUE_IS_INVALID, 'Không thể tự block chính mình');
    }
    thisUser = await User.findById(id);
    try{
        targetUser = await User.findById(user_id);
    } catch (err){
        return callRes(res, responseCode.PARAMETER_VALUE_IS_INVALID, 'Không tìm thấy ID');
    }
    if (targetUser == null){
        return callRes(res, responseCode.PARAMETER_VALUE_IS_INVALID, 'Không tìm thấy user');
    }
    if (targetUser.isBlocked){
        return callRes(res, responseCode.USER_IS_NOT_VALIDATED, 'User wanted to block has been blocked by server');
    }
    if (!targetUser){
        return callRes(res, responseCode.USER_IS_NOT_VALIDATED, 'Tài khoản không tồn tại');
    }
    else{
        ...
        if (index < 0) {
            if (type == 0){
                ...
                thisUser.save();
                targetUser.save();
                return callRes(res, responseCode.OK, 'Block thành công');
            }
            else{
                return callRes(res, responseCode.PARAMETER_VALUE_IS_INVALID, "You haven't blocked this user");
            }
        }
        else{
            if (type == 1){
                ...
                thisUser.save();
                return callRes(res, responseCode.OK, 'Bỏ chặn thành công');
            }
            else{
                return callRes(res, responseCode.PARAMETER_VALUE_IS_INVALID, "You have already blocked this user");
            }
        }
    }
});
...
router.post('/set_accept_friend', verify, async (req, res) => {
  ...
  if ( user_id === undefined|| is_accept === undefined)
    return callRes(res, responseCode.PARAMETER_IS_NOT_ENOUGH, 'user_id, is_accept');
  if (typeof user_id != 'string')
    return callRes(res, responseCode.PARAMETER_TYPE_IS_INVALID, 'user_id');
  if (!checkInput.checkIsInteger (is_accept))
    return callRes(res, responseCode.PARAMETER_TYPE_IS_INVALID, 'is_accept');
  is_accept = parseInt(is_accept, 10);
  if (is_accept != 0 && is_accept != 1)
    return callRes(res, responseCode.PARAMETER_VALUE_IS_INVALID, 'is_accept');
  let id = req.user.id;
  if (id == user_id) {
    return callRes(res, responseCode.PARAMETER_VALUE_IS_INVALID,'user_id');
  } else {
    try {
      ...
      if(!sentUser) return callRes(res, responseCode.NO_DATA_OR_END_OF_LIST_DATA, 'sentUser');
      if (is_accept == 0) {
        // xóa req bên nhận
        let indexExist = thisUser.friendRequestReceived.findIndex(element =>
          element.fromUser._id.equals(sentUser._id));
        if (indexExist < 0) return callRes(res, responseCode.METHOD_IS_INVALID, 'không có lời mời');
...
router.post("/get_list_blocks", verify, async(req, res) => {
    ...
    if (token === undefined || index === undefined || count === undefined){
        return callRes(res, responseCode.PARAMETER_IS_NOT_ENOUGH, 'token and index and count');
    }
    if (typeof index != "string"){
        return callRes(res, responseCode.PARAMETER_VALUE_IS_INVALID, 'index');
    }
    if (typeof count != "string"){
        return callRes(res, responseCode.PARAMETER_VALUE_IS_INVALID, 'count');
    }
    ...
    if (thisUser.isBlocked){
        return callRes(res, responseCode.USER_IS_NOT_VALIDATED, 'Your account has been blocked');
    }
    let code, message;
    let data = [];
    let targetUser;
...
router.post('/get_user_friends', verify, async (req, res) => {
  ...
  try {
    ...
    if (!thisUser) return callRes(res, responseCode.PARAMETER_VALUE_IS_INVALID, 'thisUser')
    // console.log(thisUser);
    if (user_id && user_id != id) {
      targetUser = await User.findById(user_id).select({ "friends": 1 });
      if (!targetUser) return callRes(res, responseCode.PARAMETER_VALUE_IS_INVALID, 'targetUser');
    ...
    if (data.friends.length == 0) return callRes(res, responseCode.NO_DATA_OR_END_OF_LIST_DATA, 'friends');
...
router.post('/get_list_suggested_friends', verify, async (req, res) => {
  ...
  if (list_users.length == 0){
    let users = await User.find({'user._id': {$ne: id}})
      ...
      .sort("-createdAt");
    if (!users) return callRes(res, responseCode.NO_DATA_OR_END_OF_LIST_DATA, 'no other user');
    for (let y of users) {
        let e = {
          user_id: y._id,
          username: y.name,
